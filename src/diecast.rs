...
pub fn get_info(url: &str, document: &Document) -> Result<Item, PodcastError> {
  ...
  let title = document.find(Class("splash-title")).next().map(|x| x.text());
  let mut date_str = document.find(Class("splash-avatar")).next().map(|x| x.text())
    .ok_or(PodcastError::new("missing avatar"))?;
  date_str = date_str.split("on ").skip(1).next()
  ...
  if let Some(temp) = document.find(Class("entry-text")).next() {
    let children: Vec<_> = temp.children().filter(|node|
      node.name() != Some("div") &&
      node.find(Name("iframe")).next() == None &&
      node.find(Name("script")).next() == None &&
      node.find(Name("audio")).next() == None).collect();
  ...
  let mp3_link = document.find(Name("audio").child(Name("source").and(Attr("type", "audio/mpeg")))).next()
    .and_then(|x| x.attr("src")).ok_or(PodcastError::new("missing mp3 link"))?;
  let mp3 = this_document.join(mp3_link)?;
  let mut response = reqwest::get(mp3.as_str())?;
  let length = response.headers().get::<ContentLength>().map(|ct_len| **ct_len).ok_or(PodcastError::new("missing mp3 length"))?.to_string();
  ...
  let itunes = ITunesItemExtensionBuilder::default()
    ...
    .image(Some("http://www.shamusyoung.com/twentysidedtale/images/splash_diecast2.jpg".to_string()))
    .build()?;
  ...
  ItemBuilder::default()
    ...
    .itunes_ext(itunes)
    .enclosure(enclosure)
    .build().map_err(|desc| PodcastError::new(&desc) )
}
