...
pub fn get_info(url: &str, document: &Document) -> Result<Item, PodcastError> {
  ...
  let mut date_str = document.find(Class("post-date-ribbon")).next().map(|x| x.text()).ok_or(PodcastError::new("missing date"))?;
  ...
  let mp3 = document.find(Name("audio").child(Name("source").and(Attr("type", "audio/mpeg")))).next().and_then(|x| x.attr("src")).ok_or(PodcastError::new("missing mp3 link"))?;
